...
import { LayoutDashboard, Leaf, Bell, LogOut, ChevronDown, Menu, X, Users } from 'lucide-react';
import { useState } from 'react';
import { mockFPO } from '../data/mockData';

const navItems = [
  { to: '/fpo/dashboard', icon: LayoutDashboard, label: 'FPO Dashboard' },
];

export default function FPOLayout({ onLogout }) {
      ...
      `}>
        <div className="flex items-center gap-3 px-6 py-5 border-b border-gray-100">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">FPO Portal</p>
          </div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `sidebar-link ${isActive ? 'bg-accent-500 text-white shadow-sm' : 'sidebar-link-inactive'}`
              }
            >
              <Icon size={18} />
              <span>{label}</span>
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50">
            <div className="w-9 h-9 bg-accent-100 rounded-full flex items-center justify-center">
              <span className="text-sm font-bold text-accent-700">KS</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 truncate">{mockFPO.name}</p>
              <p className="text-xs text-gray-500 truncate">{mockFPO.district}</p>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-gray-100">
              <div className="w-8 h-8 bg-accent-100 rounded-full flex items-center justify-center">
                <span className="text-xs font-bold text-accent-700">KS</span>
              </div>
              <span className="text-sm font-medium text-gray-700 hidden sm:block">{mockFPO.name}</span>
